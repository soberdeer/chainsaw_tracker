...
- Board view is available as a status board. Dragging a card between columns updates the OpenProject status, and the manual order inside each column is stored locally per OpenProject list/view.
...
- Global task search and list filters are backend-side, but the main list is still scoped to the selected OpenProject list/project path rather than a cross-workspace aggregate page.
...
- GitHub integration is optional and isolated from task runtime. OpenProject-backed work packages can link synced pull requests by `workPackageId`, and webhook/sync autolinking uses the extracted tracker task key when possible.
- GitHub status is currently surfaced in task detail for linked pull requests and synced head branches. Commit-by-commit activity, checks rollups on every card, and status automation remain future work.
