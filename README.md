...
- Custom fields are read from OpenProject work packages. Text-like edits are sent back as `customFieldN` PATCH values; OpenProject validation decides which field types are accepted.
...
- Import reports are stored in Prisma `MigrationRun` records when the ClickUp migration seed runs.
