...
async function main() {
  ...
  await Promise.all([
    prisma.permissionSet.upsert({
      ...
      create: { workspaceId: workspace.id, role: 'OWNER', manageWorkspace: true, manageSpaces: true, manageDocs: true, manageTasks: true, inviteMembers: true },
      update: { manageWorkspace: true, manageSpaces: true, manageDocs: true, manageTasks: true, inviteMembers: true }
    }),
    prisma.permissionSet.upsert({
      where: { workspaceId_role: { workspaceId: workspace.id, role: 'ADMIN' } },
      create: { workspaceId: workspace.id, role: 'ADMIN', manageSpaces: true, manageDocs: true, manageTasks: true, inviteMembers: true },
  ...
  const summary = await importClickUpCsv(path.resolve('prisma/seed-data/clickup-export.csv'), workspace.id);
