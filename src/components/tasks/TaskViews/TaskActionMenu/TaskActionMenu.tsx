...
export function TaskActionsMenu({ task, onChanged, onError }: {
  task: Task;
  onChanged: () => void;
  ...
  return (
    <Menu width="22rem" position="bottom-end" shadow="lg">
      <Menu.Target>
        <ActionIcon variant="subtle" aria-label="Task menu" className={classes.menuButton} onClick={(event) => event.stopPropagation()}><IconDots size="1rem"/></ActionIcon>
      </Menu.Target>
