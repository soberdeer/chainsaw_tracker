      ...
      <Stack gap="lg">
        ...
        <Group justify="space-between">
          <div>
            ...
            <Text size="sm" c="dimmed">Only invited members can open this space.</Text>
          </div>
          <Button variant="light" onClick={() => navigator.clipboard?.writeText(`${window.location.origin}/space/${space.id}`)}>Copy link</Button>
        </Group>
        <Stack gap="sm">
          ...
          {workspace.memberships.map((membership) => (
            <Group key={membership.id} justify="space-between" className={classes.personRow}>
              ...
              <Group>
                <Select size="xs" value={membership.role === 'VIEWER' ? 'View only' : 'Full edit'} data={['Full edit', 'View only']} />
                <Switch defaultChecked={membership.role !== 'VIEWER'} />
              </Group>
            </Group>
          ))}
        </Stack>
        <Button variant="light" leftSection={<IconLock size="1rem" />}>Make Public</Button>
      </Stack>
    </Modal>
  );
}
