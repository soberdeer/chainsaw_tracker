...
function githubHeaders(req: Request) {
  return {
    ...
    delivery:
      (req as { header(name: string): string | undefined }).header('x-github-delivery') || '',
  };
}
...
function isGitHubIntegrationEnabled() {
  return (
    ...
    Boolean(process.env.GITHUB_TOKEN || process.env.GITHUB_WEBHOOK_SECRET)
  );
}
...
integrationsRouter.post('/github/webhook', async (req, res) => {
  ...
  if (event === 'pull_request') {
    ...
    const requested =
      Array.isArray(prPayload.requested_reviewers) && prPayload.requested_reviewers.length > 0;
    const reviewStatus =
      payload.action === 'review_requested' || requested
        ? 'REVIEW_REQUESTED'
        : prPayload.draft
          ? 'NONE'
          : prPayload.merged
...
integrationsRouter.post('/github/repositories/:id/sync-pull-requests', async (req, res) => {
  ...
  if (!response.ok) {
    res
      ...
      .json({ error: 'GitHub API request failed', detail: await response.text() });
    return;
  }
  ...
  for (const item of pullRequests) {
    try {
      const existing = await prisma.gitHubPullRequest.findUnique({
        where: {
          repositoryId_githubPrId: { repositoryId: repository.id, githubPrId: String(item.id) },
        },
      });
      const reviewsResponse = await fetch(
        `https://api.github.com/repos/${repository.owner}/${repository.repo}/pulls/${item.number}/reviews`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            Accept: 'application/vnd.github+json',
      ...
      const reviewStatus =
        latestReview?.state === 'APPROVED'
          ...
          : latestReview?.state === 'CHANGES_REQUESTED'
            ? 'CHANGES_REQUESTED'
            : latestReview?.state === 'COMMENTED'
              ? 'COMMENTED'
              : Array.isArray(item.requested_reviewers) && item.requested_reviewers.length
                ? 'REVIEW_REQUESTED'
                : item.draft
                  ? 'NONE'
                  : 'IN_REVIEW';
...
integrationsRouter.delete(
  '/github/tasks/:taskId/pull-requests/:pullRequestId',
  async (req, res) => {
    if (!isGitHubIntegrationEnabled()) {
      sendGitHubDisabled(res);
      return;
...
integrationsRouter.post('/github/tasks/:taskId/refresh', async (req, res) => {
  ...
  const response = await fetch(
    `https://api.github.com/repos/${pullRequest.repository.owner}/${pullRequest.repository.repo}/pulls/${pullRequest.number}`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
        Accept: 'application/vnd.github+json',
