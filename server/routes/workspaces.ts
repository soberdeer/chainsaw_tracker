...
workspacesRouter.post('/:workspaceId/invites', async (req, res) => {
  ...
  const workspace = await prisma.workspace.findUniqueOrThrow({ where: { id: req.params.workspaceId } });
...
workspacesRouter.patch('/:workspaceId/memberships/:membershipId', async (req, res) => {
  ...
  const body = z.object({ role: z.enum(['OWNER', 'ADMIN', 'LEAD', 'MEMBER', 'VIEWER']) }).parse(req.body);
