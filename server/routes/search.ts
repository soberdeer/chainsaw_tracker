...
const actions = [
  { id: 'action:create-task', type: 'action', title: 'Create New Task', subtitle: 'Action', action: 'create-task' },
  { id: 'action:create-doc', type: 'action', title: 'Create New Doc', subtitle: 'Action', action: 'create-doc' },
  { id: 'action:create-space', type: 'action', title: 'Create New Space', subtitle: 'Action', action: 'create-space' },
  { id: 'action:create-folder', type: 'action', title: 'Create New Folder', subtitle: 'Action', action: 'create-folder' },
  { id: 'action:open-board', type: 'action', title: 'Open Board', subtitle: 'View', action: 'open-board' },
  { id: 'action:open-docs', type: 'action', title: 'Open Docs', subtitle: 'View', action: 'open-docs' },
  { id: 'action:open-permissions', type: 'action', title: 'Open Permissions', subtitle: 'View', action: 'open-permissions' }
] as const;
...
searchRouter.get('/', async (req, res) => {
  ...
  if (workspaceId && visibleSpaceIds.length === 0) {
    res.json(actions.filter((action) => matches(query, action.title, action.subtitle)).map((action) => ({ ...action })));
    return;
  }
