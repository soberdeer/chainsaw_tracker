...
export function computeTaskDevelopmentStatus(task: TaskWithGithub): TaskDevelopmentStatus {
  ...
  const pr =
    prs.find((item) => item.isMerged) || prs.find((item) => item.state === 'OPEN') || prs[0];

  if (pr?.isMerged || pr?.reviewStatus === 'MERGED') {
    return 'MERGED';
  }
  if (pr?.state === 'CLOSED' || pr?.reviewStatus === 'CLOSED') {
    return 'CLOSED';
  }
  ...
  if (
    pr?.state === 'OPEN' &&
    ['IN_REVIEW', 'REVIEW_REQUESTED', 'COMMENTED'].includes(pr.reviewStatus)
  ) {
    return 'CODE_REVIEW';
  }
  if (pr?.state === 'OPEN') {
  ...
  return ['in development', 'in progress', 'review', 'in review'].includes(
...
export function githubStatusMatches(task: TaskWithGithub, filter?: string) {
  ...
  if (filter === 'HAS_BRANCH') {
    return branches.length > 0 && prs.length === 0;
  }
  if (filter === 'PR_OPEN') {
    return prs.some((pr) => pr.state === 'OPEN');
  }
  if (filter === 'IN_REVIEW') {
    return dev === 'CODE_REVIEW';
  }
  if (filter === 'REVIEW_REQUESTED') {
    return prs.some((pr) => pr.reviewStatus === 'REVIEW_REQUESTED');
  }
  if (filter === 'APPROVED') {
    return dev === 'APPROVED';
  }
  if (filter === 'CHANGES_REQUESTED') {
    return dev === 'CHANGES_REQUESTED';
  }
  if (filter === 'MERGED') {
    return dev === 'MERGED';
  }
